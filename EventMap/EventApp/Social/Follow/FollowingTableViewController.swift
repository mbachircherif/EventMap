...
//  app_1
//
//  Created by BACHIR-CHERIF Mohamed on 11/07/2017.
...
class FollowingTableViewController: UITableViewController {
    ...
    func retrieveAllFollowing(){
        ...
        ref.child(uid!).observeSingleEvent(of: .value, with: { (snapshot) in
            ...
            let followingList = value?["following"] as? String
    ...
    func followAction(sender : UIButton!){
        ...
        if let cell = sender.superview as? UITableViewCell {
            ...
            if sender.titleLabel?.text == "Désabonner" {
                ...
                ref.child(uid!).updateChildValues(["following" : uidFollowingUser! ]){ (error, ref) in
                    if error != nil {
                        print("error \(String(describing: error))")
                    }
                }
...
extension FollowingTableViewController: UISearchResultsUpdating {
