//
//  followedTableView.swift
//  app_1
//
//  Created by BACHIR-CHERIF Mohamed on 03/10/2017.
...
class followedTableView: UITableViewController {
    
    let searchController = UISearchController(searchResultsController: nil)
    var following : [(uid: String, nameUser : String)] = []
    var filteredFollowing : [(uid: String, nameUser : String)] = []
    var uid : String?
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let followingArray : (uid: String, nameUser : String)
    ...
    func retrieveAllFollowing(){
        ...
        ref.child(nameProfil).observeSingleEvent(of: .value, with: { (snapshot) in
            ...
            ref.child(followingList).observeSingleEvent(of: .value, with: { (snap) in
                let followValues = snap.value as? NSDictionary
                let nameUserFollow = followValues?["nameUser"] as! String
                self.following.append((uid: followingList, nameUser : nameUserFollow))
...
extension followedTableView: UISearchResultsUpdating {
